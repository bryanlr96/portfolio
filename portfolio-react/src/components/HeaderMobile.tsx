...
export default function HeaderMobile({ headerMobileVisible, setHeaderMobileVisible }: HeaderMobileProps) {
    return (
        <div className={headerMobileVisible ? "md:hidden w-screen h-screen bg-black/80 flex flex-col fixed top-0 left-0 z-50 text-center" : "hidden"} onClick={()=> {setHeaderMobileVisible(false)}}>
            <ul className="w-full bg-blue-500 text-white">
                <li className="h-[46px] border-b-1 border-b-white flex items-center uppercase" onClick={() => {setHeaderMobileVisible(false)}}>
